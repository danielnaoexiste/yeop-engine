...
namespace yeop::input
{
	...
	void Joystick::OnJoystickConnected(SDL_ControllerDeviceEvent& e)
	{
		...
		if (SDL_IsGameController(deviceIndex))
		{
			...
			if (c->gc)
			{
				...
				YEOP_TRACE("Joystick connected: mapIndex({}), deviceIndex({})", mapIndex, deviceIndex);
				availableJoysticks[mapIndex] = std::move(c);
			}
			else
			{
				YEOP_ERROR("Error opening game controller with Device Index {}: {}", deviceIndex, SDL_GetError());
			}
		}
	}
	...
	void Joystick::OnJoystickDisconnected(SDL_ControllerDeviceEvent& e)
	{
		...
		for (auto it = availableJoysticks.begin(); it != availableJoysticks.end(); it++)
		{
			...
			{
				YEOP_TRACE("Joystick disconnected: {}", deviceIndex);
	...
	void Joystick::Update()
	{
		...
		{
			Controller* c = it->second.get();
			YEOP_ASSERT(c && c->gc, "Invalid game controller in availableJoysticks map");
			if (c && c->gc)
			{
	...
	bool Joystick::GetButton(int joystickId, Button button)
	{
		...
		YEOP_ERROR("Joystick with id {} is not available!", joystickId);
		return false;
	}
